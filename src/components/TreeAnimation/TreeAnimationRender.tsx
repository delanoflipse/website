...
import { MAX_CONTINUATION, MAX_CONTINUATIONS, MAX_DEPTH, TreeNode, getMaxContinuation } from "./TreeAnimationState";
import { NormalizedValue, asNormalized, easedValue, invert, normalise, square } from "./utils/easing";
...
const getColor = (value: number) => {
    ...
    // const startColor = [255, 0, 255]; // Yellow
...
const relativePosition = (base: Victor, direction: Victor, growth: NormalizedValue, length: number, baseSize: number): Victor => {
...
export const drawTreeNode = (ctx: CanvasRenderingContext2D, node: TreeNode, rootPos: Victor, baseSize: number): void => {
    ...
    // const growthValue = easedValue(growth);
    const tipPosition = relativePosition(rootPos, direction, growthValue, length, baseSize);

    // Draw recursive children
    for (const child of node.children) {
        const growthOfChild = child.attributes.isContinuation ? growthValue : child.properties.growAt;
        const childPosition = relativePosition(rootPos, direction, asNormalized(growthOfChild), length, baseSize);
        drawTreeNode(ctx, child, childPosition, baseSize);
    }


    // Draw node
    const maxContinuation = getMaxContinuation(depth);
    ...
    const combinedFactor = normalise(combinedDepth, MAX_CONTINUATIONS);
    const combinedFactor2 = normalise(combinedDepth + 1, MAX_CONTINUATIONS);
    ...
    // Set styling
    const gradient = ctx.createLinearGradient(rootPos.x, rootPos.y, tipPosition.x, tipPosition.y);
...
export const drawTree = (ctx: CanvasRenderingContext2D, treeRoot: TreeNode): void => {
    ...
    if (DEBUG_RENDER) {
        ...
        ctx.strokeStyle = "red";
        ctx.lineWidth = 1;
        ctx.strokeRect(drawSpace.x + drawSpace.width / 2 - rootSize / 2, drawSpace.y + drawSpace.height - rootSize, rootSize, rootSize);
    }
